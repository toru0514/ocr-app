...
- CSV列案：date / amount / vendor / account_title / tax_category / description / document_id
...
- 金額は `金額（入出金）` 1 列で入出金の符号付き金額を渡すか、`金額（入金）` `金額（出金）` 列を別々に指定する。入出金列を選んだ場合は他の金額列は選択できない。
...
| `amount` | 金額（入出金） | 税込/円。支出のみ想定のため正の値で出力し、弥生側で出金として扱う。入金列へのマッピングが必要な場合は別列を追加検討。 |
...
2024/03/16,2500,楽天ショップA,材料費,reduced_8,,6c1f7b54-...
```
- 弥生 UI では上記の `date` を「日付（年月日）」、`amount` を「金額（入出金）」、`vendor`（または `description`）を「摘要」に割り当てる。軽減税率列は `tax_category = reduced_8` の行のみ「対象」と判定できる値へ変換するロジックを実装予定。
...
2. **弥生側操作手順**（チェックリスト）
   ...
   - [ ] サンプル CSV をアップロード
   - [ ] 項目選択画面で `日付（年月日）` / `金額（入出金）` / `摘要` / `軽減税率` を割り当て
...
- **ヘッダー行**：弥生はヘッダー付き CSV を許容するため維持。ただし他ツールから出力した CSV を読み込む場合はヘッダーの有無に注意。
...
#### Phase B 実装ステップ
- [ ] **B-実装-01** 弥生 CSV の正式仕様（文字コード、ヘッダー、列マッピング）を Studio テスト結果で検証し、README を確定させる。
- [ ] **B-実装-02** `/export` の CSV 出力ロジックを求められた列順・文字コード（UTF-8 or Shift_JIS）でファイルダウンロードできるよう実装する。
